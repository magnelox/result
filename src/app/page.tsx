...
export default function StudentLandingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/result/search', {
        method: 'POST',
      ...
      // Store result in sessionStorage for instant rendering
      sessionStorage.setItem('ssu_student_result', JSON.stringify(data.data));
      router.push('/result');
    } catch (err) {
      setErrorMessage(
        "We couldn't find a result with those details. Please check your Registration Number, Date of Birth and Programme and try again."
    ...
    <div className="max-w-2xl mx-auto py-4">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200/80 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-ssu-navy via-ssu-navy-light to-ssu-navy p-8 text-white text-center border-b-4 border-ssu-gold relative">
          <span className="inline-block bg-ssu-gold/20 text-ssu-gold-light text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-3 border border-ssu-gold/30">
          ...
          <p className="text-xs text-slate-300 mt-2 font-light">
            View your published semester examination grade sheet
          </p>
        </div>

        {/* Calm Loading State Overlay when submitting */}
        {searching ? (
          <div className="p-12 text-center space-y-4">
            ...
            <h3 className="text-lg font-serif font-bold text-ssu-navy">Checking your result...</h3>
            <p className="text-sm text-slate-600 max-w-sm mx-auto">
              Please wait while we locate your official published examination records.
            </p>
          </div>
        ) : (
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Enter your Registration Number (e.g. 2026MBA001)"
                  className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-300 rounded-lg text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-ssu-navy focus:border-ssu-navy focus:bg-white text-sm transition-all"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-10 pr-10 py-3 bg-slate-50 border border-slate-300 rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-ssu-navy focus:border-ssu-navy focus:bg-white text-sm transition-all appearance-none"
                >
                  <option value="">
      ...
      {/* Official SSU Information Card */}
      <div className="mt-8 bg-blue-50/60 border border-blue-200/80 rounded-xl p-5 text-xs text-blue-900 space-y-1.5">
        <h4 className="font-bold font-serif text-sm text-ssu-navy flex items-center gap-1.5">
          <span>ℹ️</span> Notice for Students
        </h4>
        <p>
          Results displayed here represent official published grade sheets for the current semester examination session.
        </p>
        <p>
          If your result is not yet available, please verify your details with your Department Coordinator or check back after official announcement.
        </p>
      </div>
    </div>
  );
